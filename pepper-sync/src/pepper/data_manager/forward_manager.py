...
class ForwardManager:
    ...
    def __init__(
        self, device_manager: DeviceManager, position_manager: PositionManager
    ) -> None:
    ...
    def process(self) -> list[ForwardObject]:
        ...
        cached_positions = (
            cache_position.get_all()
        )  # self.position_manager.get_by_resolver()
        ...
        ids = ", ".join(
            str(p.name)
            for p in items
            if hasattr(p, "unique_id") and p.unique_id is not None
        )
        logger.info(f"Forwarding payload for {len(items)} devices")
        logger.info(f"Forwarding includes: {ids}")
